<template>
    <div>
        <Header ref="header" pageTitle="Admin Orders" :hide_settings_icon="true"/>
        <div class="page-content">
            Date début<input type="date" name="">
            Date fin<input type="date" name="">
            ...
            <div v-show="order_tab_active" style="min-height: 200px">
                <div v-for="(line, index) in line_orders">
                    <div v-if="!line.cancelled_at && line.id!=2319" class="card card-style mb-2" style="padding: 10px 10px;margin: 0 15px 20px;border-radius: 10px;">
                        <div>
                        ...
                        <div>
                            <router-link :to="'/store/'+user.id" style="text-align: left;width: 49%;display: inline-block;color: #000;">
                        ...
                        <div class="d-flex">
                            <div>
                                <img @click="$router.push('/product/'+line.product.id)" :src="line.product.imageFirst" class="rounded-sm" width="100" style="border:1px solid lightgray;">
                            </div>
                        ...
                        <div class="d-flex">
                            ...
                            <div class="w-100 ms-3 pt-1">
                                ...
                                <div class="row" style="margin-bottom: 0px;">
                                    <div class="col-12">
                                        <router-link class="font-700 btn btn-dark-piketplace rounded-xl" :to="{ name: 'shipping_management', params: { id: line.id } }" style="font-size: 12px;margin-right: 0;width: 100%;padding: 2px;"><i class="fas fa-shipping-fast"></i> {{$t('message.confirmation_text')}}</router-link>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    export default {
        ...
        methods: {
            ...
            async loadOrders() {
                ...
                await axios.get('/admin-orders', {params: {page}})
                .then(res => {
            ...
            async loadOrdersShipped() {
                ...
                let user_id = this.user.id;
                let page = this.current_page;
                await axios.get('/admin-shipped-line-orders', {params: {user_id, page}})
                .then(res => {
                    this.isLoading = false;
                    this.data_pagination = res.data.line_orders;
            ...
            pay_piketplace_wallet(){//Payment on preorder
                if (this.user.balance < this.total) {
                    this.$functions.msg_box(this, 'error', 'Info', this.$t('message.donate.not_enough_amount',{'amount': this.user.balance}))
                    return 0;
                }
            ...
            pay_piketplace_call(){//Payment on preorder
