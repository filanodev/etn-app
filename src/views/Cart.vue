    ...
    <div style="">
        ...
        <div id="select-address" class="menu menu-box-bottom rounded-m" data-menu-height="400" data-menu-effect="menu-over" style="display: block; height: 365px;">
            <div class="menu-title">
                <h1 class="font-24 color-piketplace pb-0">
                    ...
                    &nbsp;<span class="btn btn-xxs mb-1 rounded-xl text-uppercase font-900 shadow-s btn-dark-piketplace align-middle" @click="$router.push({name: 'my-addresses'})"><i class="fa fa-address-card"></i></span>
                </h1>
        ...
        <div class="page-content">
            ...
            <div v-if="isLoggedIn && cart.length > 0">
                <div class="card card-style mb-2" v-for="(c,index) in cart" :key="index" :id="'content-'+c.product.id" style="padding: 20px 15px;">
                    ...
                    <div class="d-flex">
                        ...
                        <div>
                            <img @click="$router.push({ name: 'product', params: { id: c.product.id } })" :src="c.product.imageFirst" class="rounded-sm" width="150" style="border:1px solid lightgray;">
                        </div>
                    ...
                    <div v-if="!c.direct_paid_shipping && !c.direct_free_shipping && !c.noshipping">
                        <h4 class="font-12 font-900" style="line-height: 18px;" v-if="(c.in_free_shipping_zone=='no' && c.in_a_paidzone_city) || (c.in_a_paidzone_city && !c.in_a_freezone_city)"><!-- Are you in paid shipping zone ? {{c.paid_shipping_info.zone}} -->
                        ...
                        <div class="row mt-0 mb-0 me-3 ms-3" v-if="(c.in_free_shipping_zone=='no' && c.in_a_paidzone_city) || (c.in_a_paidzone_city && !c.in_a_freezone_city)">
                            ...
                            <div class="col-12" style="text-align: left;margin-left: -20px;" v-if="c.in_paid_shipping_zone=='yes'">
                                <h3 class="font-600" style="line-height: 18px;">{{$t('message.choose_shipping_zone')}}</h3>
                                <div style="background-color: lightgray;color: black;padding: 10px 10px 10px 20px;border-radius: 10px;margin: 0px 0px;display: inline-block;">
                                    <ul style="list-style-type:circle;">
                                        <li v-for="zone in c.product.shipping_zone.filter(x=>x.country_code==address.country_code)" @click="zone_select(index, zone)" style="line-height: 18px;">
            ...
            <div style="text-align: center;" v-else-if="cart.length == 0 && isLoggedIn">
                <a class="no_more_data color-piketplace">{{ $t('message.cart.empty_cart') }}</a>
            </div>
...
<script>
    ...
    export default{
        ...
        computed: {
            ...mapWritableState(useStore, ['isLoggedIn', 'user', 'uniqueId', 'locale', 'prevRoute', 'error', 'connecting', 'deleting', 'maintenance_mode', 'isOpenLoading', 'purchase_activation', 'country_cities', 'payment_from', 'at_least_one_product_qty_insufficient', 'countries_db', 'isLoadingCities', 'country_cities','saving', 'pi_balance', 'piket_balance', 'activate_google_ads', 'piket_wallet_frontend_url']),
        },
        ...
        methods: {
            ...mapActions(useStore, ['login', 'clearPaymentVerifier', 'initLoaders', 'launchPaymentVerifier', 'piPayment', 'getCities', 'getCountryNameByCode', 'setUniqueId', 'showPiAds', 'setBalance', 'is_user_profil_set']),
            search_city(){
                this.cities = this.country_cities
            ...
            loadDefaultAddress(){
                ...
                if (this.isLoggedIn && this.addresses) {
                    ...
                    //console.log('this.address test', this.address, this.addresses)
            ...
            pay_piketplace_wallet(code_pin){
                ...
                if (this.pi_balance < this.total) {
                    this.$functions.msg_box(this, 'error', 'Info', this.$t('message.donate.not_enough_amount',{'amount': this.$functions.amount_format(this.pi_balance, this.$currency)}))
                    return 0;
                }
