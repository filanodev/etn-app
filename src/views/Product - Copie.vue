    ...
    <div class="page-content">
        ...
        <div v-if="!isLoading && product && (isOwner || !product_unavailable)">
            ...
            <div style="margin: 5px;padding: 5px;">
                <i v-can="'validate_products'" v-if="product.status!='rejected'" class="fa fa-trash-o color-red-dark cursor-pointer align-middle" @click="validation_status='rejected';validate_product(product.id)" style="font-size: 18px;">&nbsp; Rejeter produits</i>
                <i v-can="'validate_products'" v-if="product.status!='validated'" class="fa fa-check color-green-dark cursor-pointer align-middle" @click="validation_status='validated';validate_product(product.id)" style="font-size: 18px;">&nbsp; Valider produits</i>
                    ...
                    <div class="mb-0">
                        <button class="btn accordion-btn no-effect color-theme collapsed" data-bs-toggle="collapse" data-bs-target="#collapse2" aria-expanded="false">
                            <div class="content" style="display: inline-block;margin: 0;">
                                <p class="mb-n1 color-piketplace font-600 mb-n1">{{$t('message.boost')}} <i class="fa fa-rocket"></i> </p>
                            </div>
                        ...
                        <div id="collapse2" class="collapse show" data-bs-parent="#accordion-1" style="">
                            <div class="pt-1 pb-2 ps-3 pe-3">
                                <div class="content mt-0" v-if="!is_upgrade && already_boosted && product.last_boost">
                                    <h2 class="font-14 color-piketplace text-center">{{$t('message.boost_for_on', {amount: $functions.amount_format(product.last_boost.amount, product.last_boost.currencies_code), period: $t('message.'+product.last_boost.period)})}} <div @click="is_upgrade=true" class="btn btn-dark-piketplace" style="display: inline;width: 45px;">
            ...
            <div class="" style="display: flex;justify-content: center;">
                <div class="card card-overflow card-style" :class="isOwner && approbation_active?'ribbon':''" style="margin-top: -62px;z-index: 0;width: 100%;">
                    <div class="card__containerrr">
                        ...
                        <h5 @click="$router.push({ name: 'store', params: { shop_user_id: product.user.id } })" class="mb-0" style="float: right;">
                        ...
                        <h1 class="mb-0 pb-0 font-600" style="line-height: 20px;">{{product.libelle}}</h1>
                        ...
                        <div v-if="!product.is_digital" class="row mb-0">
                            ...
                            <div class="col-8">
                                <a class="btn btn-m font-600 rounded-s shadow-l btn-dark-piketplace" @click="axios_adding()" style="margin-right: 2px;margin-top: 5px;">{{$t('message.product.add')}}<i class="fa fa-shopping-cart color-white ps-2"></i></a>
                            ...
                            <div class="pt-1 pb-1 ps-3 pe-3">
                                <div class="content mt-0 mb-1">
                                    <div v-for="zone, index in product.shipping_zone" class="vcard-field" style="line-height: 24px;padding-top: 15px;">
                                      ...
                                      <i class="fa fa-map-marker color-blue2-dark mt-n2 opacity-1"></i>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="card card-style" v-if="product.imageFirst != undefined && product.images.length > 1">
                <div class="content mb-0">
                    <p class="mb-n1 color-piketplace font-600 mb-n1">{{$t('message.product.view_in_detail')}} {{product.images.length}}</p>
                    <h2>{{$t('message.product.product_gallery')}}</h2>
                    <div class="card card-style bg-18" data-card-height="300" v-for="image in product.images">
            ...
            <div class="card card-style" v-if="product && product.comments.length > 0">
                <div class="accordion" id="accordion-1">
                    <div class="mb-0">
                        <button class="btn accordion-btn no-effect color-theme collapsed" data-bs-toggle="collapse" data-bs-target="#collapse2" aria-expanded="false">
                            <div class="content" style="display: inline-block;margin: 0;">
                                <p class="mb-n1 color-piketplace font-600 mb-n1">{{$t('message.product.what_pioneers_say')}} </p>
